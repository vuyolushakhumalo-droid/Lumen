...
const EMBED_ALLOWLIST = [
  ...
  // Memberstack's install snippet is a single script tag with data
  // attributes and no inline JS. v1 (/scripts/v1/memberstack.js) and
  // v2 (/scripts/v2/) both come from this host, so the host-level
  // entry covers a customer on either version.
  'static.memberstack.com',
  // food ordering
  // GloriaFood issues its "See MENU & Order" snippet from two hosts
  // depending on which of its own help pages the customer followed --
  // fbgcdn.com ("FoodBooking Global CDN") in the Squarespace and
  // button-customisation guides, foodbooking.com in the general
  // install guide. Both serve ewm2.js and both are live, so both are
  // listed: allowing only one silently breaks half of the snippets
  // GloriaFood itself hands out.
  'www.fbgcdn.com',
  'www.foodbooking.com',
];
...
//
// EMBED_ALLOWLIST carries www.youtube-nocookie.com and deliberately not
// www.youtube.com, so an embed cannot set cookies on a customer's
// visitors. But YouTube's own Share -> Embed gives www.youtube.com
// unless the visitor ticks "privacy-enhanced mode", and the build
// prompt tells the model to paste a customer's snippet verbatim. The
// result was that following YouTube's own instructions got the site
// refused at publish, or taken offline on the next edit.
//
// So the URL is rewritten to the no-cookie host instead of the host
// being allowed. The privacy intent is kept; the customer stops paying
// for it. Only YouTube hosts are touched, and only inside an iframe
// src -- an <a href> to a video is a link, not an embed, and rewriting
// it to /embed/ would send visitors to a bare player.
// ============================================================
const YT_PAGE_HOSTS = new Set(['youtube.com', 'www.youtube.com', 'm.youtube.com']);
const YT_SHORT_HOSTS = new Set(['youtu.be', 'www.youtu.be']);
const YT_NOCOOKIE_HOST = 'www.youtube-nocookie.com';

// YouTube ids are 11 chars today. The range is deliberately loose, but
// bounded: it has to reject a path segment that is really a channel
// name or a slug, or /watch on a playlist would become a broken embed.
const YT_VIDEO_ID = /^[A-Za-z0-9_-]{8,16}$/;

/**
 * The no-cookie embed URL for a YouTube URL that was used as an embed,
 * or null to leave the value exactly as it is.
 *
 * Returns null for every non-YouTube host, for a YouTube URL that is
 * not a video (a channel, a playlist page, the home page), and for
 * anything unparseable. Query parameters are carried over untouched.
 */
function nocookieEmbedUrl(raw) {
  let value = String(raw == null ? '' : raw).trim();
  if (!value) return null;

  // A snippet pasted into HTML may carry &amp; in the query string.
  // Parse the decoded form and put the escaping back on the way out.
  const wasEscaped = value.includes('&amp;');
  if (wasEscaped) value = value.replace(/&amp;/g, '&');

  // "//host/path" inherits the page's scheme.
  const wasProtocolRelative = value.startsWith('//');
  if (wasProtocolRelative) value = 'https:' + value;
  ...
  if (YT_PAGE_HOSTS.has(host)) {
    const embed = url.pathname.match(/^\/embed\/([^/]+)\/?$/);
    if (embed) {
      id = embed[1];
    } else if (url.pathname === '/watch' || url.pathname === '/watch/') {
      id = params.get('v');
      params.delete('v');                           // it becomes the path
    }
  } else if (YT_SHORT_HOSTS.has(host)) {
    const short = url.pathname.match(/^\/([^/]+)\/?$/);
    if (short) id = short[1];
  } else if (host === 'youtube-nocookie.com') {
    // Right host, missing the www. the allowlist matches on, so it
    // would be blocked on a technicality.
    const embed = url.pathname.match(/^\/embed\/([^/]+)\/?$/);
    if (embed) id = embed[1];
  } else {
    return null;                                    // not YouTube -- never touched
  }

  if (!id || !YT_VIDEO_ID.test(id)) return null;

  url.protocol = 'https:';
  url.hostname = YT_NOCOOKIE_HOST;
  url.port = '';
  url.pathname = `/embed/${id}`;
  url.search = params.toString() ? `?${params.toString()}` : '';

  let out = url.toString();
  if (wasProtocolRelative) out = out.replace(/^https:/, '');
  if (wasEscaped) out = out.replace(/&/g, '&amp;');
  return out;
}

/**
 * Rewrites YouTube iframe sources to the no-cookie host. Everything
 * else in the document is returned byte-for-byte.
 *
 * Runs before findHardBlock everywhere that screen runs, and before
 * generated code is saved, so what is stored and served is what was
 * screened.
 */
export function normalizeEmbeds(html) {
  ...
  return s.replace(/<iframe\b[^>]*>/gi, (tag) =>
    tag.replace(/(\ssrc\s*=\s*)(["'])([^"']*)\2/i, (whole, prefix, quote, value) => {
      const rewritten = nocookieEmbedUrl(value);
...
const HARD_BLOCK_PATTERNS = [
  {
    ...
    fix: "If you wanted a members' area or a client login, sign up at memberstack.com, add your site there, and paste me the snippet it gives you — I'll wire it in and gate whichever sections you choose.",
  ...
  },
  {
    ...
    // Live since the audit came back clean across every published site.
    // The experimental gate it used to carry stays in findHardBlock for
    // the next rule that needs proving before it can unpublish anyone.
...
/**
 * The host a hard-block hit was about, or null when the rule has no URL
 * behind it (a password field is not about a site). Used to name the
 * offending site in the message the customer reads.
 */
export function hardBlockHost(hit) {
  if (!hit || !hit.detail) return null;
  let value = String(hit.detail).trim();
  if (value.startsWith('//')) value = 'https:' + value;
  try {
...
export function findHardBlock(html, options = {}) {
  ...
  for (const rule of HARD_BLOCK_PATTERNS) {
    ...
    // good ones still blocks. The offending URL is carried into the hit
    // so the customer can be told which site was refused -- "an embedded
    // page from a website we do not recognise" alone does not tell them
    // which one to remove.
    rule.re.lastIndex = 0;
    for (const m of s.matchAll(rule.re)) {
      if (!rule.allow(m)) return { ...hit, detail: m[2] };
    }
  }
  return null;
}
...
export async function screenLiveSite(admin, { projectId, code }) {
  // Normalised first, so an edit that pastes YouTube's own embed snippet
  // does not take a working site offline. The generate routes normalise
  // before they save, so this repeats that for content saved earlier.
  const hit = findHardBlock(normalizeEmbeds(code));
  if (!hit) return null;
