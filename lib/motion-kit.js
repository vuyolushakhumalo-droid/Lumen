// ============================================================
// Lintel Motion Kit v0.1 — the CSS and JS every generated site gets, so
// sites take their movement from one tested library rather than animation
// code the model improvises each time. The model only writes two markers;
// insertMotionKit (below) puts the kit in after generation.
//
...
// ------------------------------------------------------------
// Markers. The model never writes the kit's code: it marks where the kit
// goes and the server swaps the markers for the kit after generation and
// the hard-block screen. Saved pages carry the expanded kit, tagged
// data-lintel-kit; edits fold it back to markers before the model sees
// the page, so the model never patches it and every edit gets the
// current kit.
// ------------------------------------------------------------
export const MOTION_KIT_CSS_MARKER = '<!--LINTEL-KIT-CSS-->';
export const MOTION_KIT_JS_MARKER = '<!--LINTEL-KIT-JS-->';

const CSS_MARKER_RE = /<!--\s*LINTEL-KIT-CSS\s*-->/i;
const JS_MARKER_RE = /<!--\s*LINTEL-KIT-JS\s*-->/i;
const KIT_STYLE_RE = /<style data-lintel-kit>[\s\S]*?<\/style>/gi;
const KIT_SCRIPT_RE = /<script data-lintel-kit>[\s\S]*?<\/script>/gi;
const USES_KIT_RE = /\sclass\s*=\s*["'][^"']*\blk-/i;

const kitStyle = () => `<style data-lintel-kit>\n${MOTION_KIT_CSS}\n</style>`;
const kitScript = () => `<script data-lintel-kit>\n${MOTION_KIT_JS}\n</script>`;

// Where an element still open at `index` starts, or -1. Catches a marker
// written inside the page's own <style> or <script>, where the kit would
// otherwise be nested inside that element and break both.
function openElementStart(html, index, tag) {
  const before = html.slice(0, index).toLowerCase();
  const open = before.lastIndexOf(`<${tag}`);
  return open > before.lastIndexOf(`</${tag}`) ? open : -1;
}

// Puts `block` where the first marker is and drops any repeats, or
// returns null when there is no marker.
function placeAtMarker(html, markerRe, tag, block) {
  const first = markerRe.exec(html);
  if (!first) return null;
  const stripped = html.replace(new RegExp(markerRe.source, 'gi'), '');
  const inside = openElementStart(stripped, first.index, tag);
  const at = inside >= 0 ? inside : first.index;
  return stripped.slice(0, at) + block + stripped.slice(at);
}

function insertBefore(html, re, block) {
  const at = html.search(re);
  return at < 0 ? html : html.slice(0, at) + block + '\n' + html.slice(at);
}

function insertBeforeLast(html, needle, block) {
  const at = html.toLowerCase().lastIndexOf(needle);
  return at < 0 ? html + block : html.slice(0, at) + block + '\n' + html.slice(at);
}

/**
 * Replaces the markers with the kit. A page that uses kit classes but is
 * missing a marker still gets that half of the kit, since its sliders and
 * grids fall apart without it. `css` and `js` say whether each marker was
 * there, for the caller's warning.
 */
export function insertMotionKit(html) {
  let out = String(html || '');
  const usesKit = USES_KIT_RE.test(out);

  const withCss = placeAtMarker(out, CSS_MARKER_RE, 'style', kitStyle());
  if (withCss != null) out = withCss;
  else if (usesKit) out = insertBefore(out, /<style\b|<\/head>/i, kitStyle());

  const withJs = placeAtMarker(out, JS_MARKER_RE, 'script', kitScript());
  if (withJs != null) out = withJs;
  else if (usesKit) out = insertBeforeLast(out, '</body>', kitScript());

  return { html: out, css: withCss != null, js: withJs != null, usesKit };
}

/**
 * The reverse, for edits: folds a saved page's kit back into its markers.
 * Also lifts out a kit pasted in full by builds from before markers
 * existed, so those sites can't end up carrying two copies.
 */
export function collapseMotionKit(html) {
  let out = String(html || '')
    .replace(KIT_STYLE_RE, MOTION_KIT_CSS_MARKER)
    .replace(KIT_SCRIPT_RE, MOTION_KIT_JS_MARKER);

  if (!CSS_MARKER_RE.test(out) && out.includes(MOTION_KIT_CSS)) {
    out = insertBefore(out.replace(MOTION_KIT_CSS, ''), /<style\b/i, MOTION_KIT_CSS_MARKER);
  }
  if (!JS_MARKER_RE.test(out) && out.includes(MOTION_KIT_JS)) {
    const lifted = out.replace(MOTION_KIT_JS, '').replace(/<script>\s*<\/script>\s*/i, '');
    out = insertBeforeLast(lifted, '</body>', MOTION_KIT_JS_MARKER);
  }
  return out;
}
