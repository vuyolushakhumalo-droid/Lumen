...
const DESIGN_DIRECTIONS = [
  ...
  {
    name: 'Cinematic',
    brief: `Set headings in ONE large editorial display face — a condensed grotesk or a high-contrast display serif — at very large sizes with tight tracking (letter-spacing around -0.02em to -0.04em), and keep body copy in a quiet neutral sans. The page sits on near-black or a deep tone (ink, midnight, deep forest, oxblood) with light text. The hero is full-bleed and fills the viewport: either a generated photograph with the product or brand object as its subject, or a slow, video-style CSS gradient in motion behind the headline. Treat that product or brand imagery as the hero object — large, isolated and dramatically lit, never a small picture beside the copy. Build layered depth with CSS transforms: type and imagery on separate layers with a gentle parallax on scroll, and a subtle reveal-on-scroll (a short fade and rise via IntersectionObserver). Add the hidden starting state from the script, so nothing is invisible if it fails, and under prefers-reduced-motion drop the parallax and reveals entirely. Leave generous negative space — few elements per viewport and long gaps between sections. Colour is restrained: the dark base, light neutrals and a single accent used on one button or detail at a time.`,
  },
];
