...
function hardBlockRetryNote(block) {
  return [
    'IMPORTANT — your previous attempt at this change was rejected automatically and cannot be published.',
    `It contained ${block.label}${block.detail ? ` (${block.detail})` : ''}.`,
    'Make the requested change again WITHOUT it.',
...
async function runEdit(args) {
  const first = await editOnce(args);

  const block = findHardBlock(first.html);
  if (!block) return first;

  console.warn('[generate] edit tripped a hard block — retrying once', {
    projectId: args.projectId,
    ruleId: block.id,
    detail: block.detail || null,
    editMode: first.editMode,
  });

  const retry = await editOnce({ ...args, blockNote: hardBlockRetryNote(block) });
  const usage = addUsage(first.usage, retry.usage);
  const stillBlocked = findHardBlock(retry.html);

  if (stillBlocked) {
    // Give up and save it. The normal offline path takes over from
    // here: the site comes down and the customer is told why, which is
    // the behaviour we already have for content we can't publish.
    console.error('[generate] retry still blocked — saving and letting the offline path handle it', {
      projectId: args.projectId,
  ...
  console.warn('[generate] retry cleared the block', {
    projectId: args.projectId,
    ruleId: block.id,
  });
