...
import { MOTION_KIT_CSS, MOTION_KIT_JS } from './motion-kit.js';
...
// Every new build gets the motion kit, after the direction block (which
// can name kit classes). It is identical on every build, so it carries
// its own cache breakpoint: one cached copy per direction.
const MOTION_KIT_BLOCK = `Motion kit

Lintel has its own motion library, the motion kit. All movement on the site comes from it: use its classes, and never write your own scroll, reveal, parallax, tilt, counter, slider or accordion code, never add @keyframes of your own, and never load an animation library. The one exception is a design direction above that explicitly asks for an animated CSS gradient, which you may write as a single @keyframes.

Include the kit exactly once, verbatim and character for character — both parts, however many patterns you use:
- The kit CSS goes at the very top of the page's single <style> tag, before the site's own rules, so your rules can resize and restyle kit elements (for example .lk-hero{aspect-ratio:auto;min-height:100svh;border-radius:0} for a full-bleed hero).
- The kit JS goes in its own separate <script> element immediately before </body>, after the site's navigation script, so an error in one can never stop the other.
If the brief asks for no motion or animation, leave the kit out entirely.
...
The kit already turns every pattern off under prefers-reduced-motion, so add no reduced-motion rules of your own for it. You may set --lk-accent to the site's accent colour in your own CSS.

Kit CSS (copy everything between the tags, not the tags themselves):
<motion-kit-css>
${MOTION_KIT_CSS}
</motion-kit-css>

Kit JS (copy everything between the tags, not the tags themselves):
<motion-kit-js>
${MOTION_KIT_JS}
</motion-kit-js>`;
...
// Logged, not enforced: says how reliably builds carry the kit intact.
function noteMotionKit(html, projectId) {
  const css = html.includes(MOTION_KIT_CSS);
  const js = html.includes(MOTION_KIT_JS);
  if (!css || !js) console.warn('[generate] build is missing the motion kit verbatim', { projectId, css, js });
}
...
async function runEdit(args) {
  return withHardBlockRetry(
    ({ blockNote }) => editOnce({ ...args, blockNote }),
