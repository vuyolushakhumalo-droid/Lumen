 ...
 */
export async function generateSite({ brief, modelKey = 'sonnet', previousHtml = null }) {
  const editing = !!previousHtml;

  const messages = editing
    ? [{ role: 'user', content: `Here is the current website:\n\n${previousHtml}\n\n---\n\nRequested change: ${brief}` }]
    : [{ role: 'user', content: `Build a website. Brief: ${brief}` }];
 ...
 */
export async function streamSite({ brief, modelKey = 'sonnet', previousHtml = null, onChunk, signal }) {
  const editing = !!previousHtml;
