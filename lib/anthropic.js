...
import { findHardBlock } from './publish.js';
import { MOTION_KIT_CSS_MARKER, MOTION_KIT_JS_MARKER, insertMotionKit, collapseMotionKit } from './motion-kit.js';
...
// These mirror the hard-block rules in lib/publish.js. A site that
// breaks them is refused at publish, or taken offline if it was already
// live -- so this is not style guidance, it is the difference between a
// working site and a customer discovering theirs has gone dark.
const EXTERNAL_CODE_RULES = `## Code that reaches outside the page
Never write inline JavaScript that sends data to another website. No fetch(), XMLHttpRequest, navigator.sendBeacon() or WebSocket pointing at an outside address. Requests to a path on the site's own domain are fine.
Never add a <script src="..."> or an <iframe src="..."> pointing at a host the request did not give you. Analytics, tag managers, pixels, heatmaps, chat widgets, A/B tools and session recorders are all refused when the site is published, so adding one costs the customer a working site rather than gaining them a feature.
Never include a password field, and never point a form's action at another company — form submission is wired automatically and must stay that way.
If the customer asks for a third-party feature — live chat, online booking, analytics, a shop, a reviews widget — do NOT approximate it and do NOT guess the provider's URLs or script tags. Build the section where it belongs, leave a clear placeholder in it, and use the plan comment's "message" to tell them to paste the provider's own embed snippet from their account. You will insert that snippet verbatim when they give it to you. Saying "paste me the snippet from your Calendly account and I'll add it" is a better answer than a booking form that only looks real.
Members-only content, a client login, a members' area or paid access all need a real membership service — you cannot build sign-in yourself, and a password box on a generated page protects nothing. Build the gated section with its real content in place, and in the plan comment's "message" recommend Memberstack by name: they sign up at memberstack.com, add their site, and paste the snippet it gives them, which you will then insert verbatim. Once that snippet is in, gating is done with data attributes on the elements themselves (for example data-ms-content="members" on a members-only section) — never with a password field.`;
...
Optionally add "class" with space-separated class names for the image, when it needs motion kit classes — for example {"prompt":"...","aspect":"landscape","alt":"...","class":"lk-hero-img lk-parallax"}. The class is carried onto the finished <img>.
...
// Six design directions. One is picked per NEW build and appended to the
...
const DESIGN_DIRECTIONS = [
  ...
  {
    name: 'Cinematic',
    brief: `Set headings in ONE large editorial display face — a condensed grotesk or a high-contrast display serif — at very large sizes with tight tracking (letter-spacing around -0.02em to -0.04em), and keep body copy in a quiet neutral sans. The page sits on near-black or a deep tone (ink, midnight, deep forest, oxblood) with light text. The hero is full-bleed and fills the viewport: either a generated photograph with the product or brand object as its subject, built on the motion kit's lk-hero with the photo as lk-hero-img lk-parallax (sized to the full viewport with your own CSS), or a slow, video-style CSS gradient in motion behind the headline. Treat that product or brand imagery as the hero object — large, isolated and dramatically lit, never a small picture beside the copy. Build layered depth: type and imagery on separate layers, the hero image lagging behind on scroll, and headings, images and cards below the hero revealed subtly as they scroll in with lk-reveal. Leave generous negative space — few elements per viewport and long gaps between sections. Colour is restrained: the dark base, light neutrals and a single accent used on one button or detail at a time.`,
  },
];

export const DESIGN_DIRECTION_NAMES = DESIGN_DIRECTIONS.map((d) => d.name);

// Cinematic can be chosen explicitly on any plan, but is only picked
// automatically on these — it is the most ambitious direction to render.
const CINEMATIC_DEFAULT_PLANS = ['frontier', 'studio'];
...
function pickDesignDirection(brief = '', plan = null) {
  const b = String(brief).toLowerCase();
  const cinematicByDefault = CINEMATIC_DEFAULT_PLANS.includes(plan);
  const rules = [
  ...
  if (cinematicByDefault) rules.push([/\b(fashion|brand|product|studio|agency|tech|launch)/, 'Cinematic']);

  for (const [re, name] of rules) {
    if (re.test(b)) return { direction: DESIGN_DIRECTIONS.find((d) => d.name === name), path: 'keyword' };
  }
  const pool = cinematicByDefault ? DESIGN_DIRECTIONS : DESIGN_DIRECTIONS.filter((d) => d.name !== 'Cinematic');
  return { direction: pool[Math.floor(Math.random() * pool.length)], path: 'random' };
}

// A brief that already says how the site should look — fonts, colours,
// layout, motion, "style like…" — must not have a direction layered on
// top of it, or the direction quietly overrides what the customer asked
// for. Deliberately simple: a false positive only costs a random style.
const DESIGN_INSTRUCTION_RE = /\b(fonts?|typefaces?|typography|serif|colou?rs?|palette|layout|animat(?:ed|ion|ions)|motion|parallax)\b|#[0-9a-f]{6}\b|#[0-9a-f]{3}\b|\b(?:styled?|looks?) like\b|\bin the style of\b/i;

function hasExplicitDesignInstructions(brief = '') {
  return DESIGN_INSTRUCTION_RE.test(String(brief));
}

// Decided once per build, not per attempt, so a hard-block retry keeps
// the same direction as the attempt it replaces. Returns null when the
// brief's own design instructions should stand alone.
function chooseDesignDirection({ brief, style, plan, projectId }) {
  let direction = null;
  let path;
  const selected = style ? DESIGN_DIRECTIONS.find((d) => d.name === style) : null;
  if (selected) {
    direction = selected;
    path = 'selected';
  } else if (hasExplicitDesignInstructions(brief)) {
    path = 'brief_instructions';
  } else {
    ({ direction, path } = pickDesignDirection(brief, plan));
  }
  console.log('[generate] design direction', { projectId, path, direction: direction?.name || null, plan: plan || null });
  return direction;
}
...
function designDirectionBlock(d) {
  return `Design direction for this build: ${d.name}

${d.brief}

Apply this direction to type, hero layout, spacing, colour and imagery. It shapes the style only — it never overrides the output format, structure, form, icon or metadata rules above. Where the brief itself names fonts, colours, layout or motion, follow the brief over this direction.`;
}

// What the eight patterns are for, shared by the build and edit prompts.
// The model never sees or writes the kit's code: it marks where the kit
// goes, and insertMotionKit (lib/motion-kit.js) puts it there afterwards.
const MOTION_KIT_PATTERNS = `The eight patterns:
- lk-reveal: fades and rises an element once as it scrolls into view, with data-delay="120" to stagger siblings — use on headings, cards and images below the hero, never on the navigation or the hero itself.
- lk-hero, lk-hero-img, lk-hero-copy and lk-parallax: a hero photo that settles from a slight zoom and drifts slowly with copy overlaid; add lk-parallax to the photo so it lags behind on scroll (lk-parallax also works on one large feature image inside a positioned, overflow-hidden frame, but never on the same element as lk-reveal or lk-tilt) — use for photo-led, full-bleed heroes.
- lk-tilt: a card leans toward the cursor on desktop and stays still on touch — use sparingly, on a few product, portfolio or pricing cards.
- lk-marquee containing lk-track: a slow, seamless horizontal loop that pauses on hover; write the items twice inside the track, with aria-hidden="true" on each repeat — use for stockists, client names, services, press mentions or areas covered.
- lk-count: a number that counts up when it comes into view, as <b class="lk-count" data-to="1200">1,200</b>; always write the real number inside, and add data-plain for years so 1962 never shows as 1,962 — use for founding years, review counts, star ratings and jobs completed.
- lk-compare: a draggable before/after slider, as <div class="lk-compare"> holding the before image (class "before"), the after image (class "after"), <span class="lbl" style="left:12px">Before</span><span class="lbl" style="right:12px">After</span><div class="bar"></div><input type="range" min="0" max="100" value="50" aria-label="Compare before and after"></div> — use for trades and services with before-and-after photos: renovation, cleaning, landscaping, roofing, detailing.
- lk-acc: an accordion on native details/summary, as <div class="lk-acc"><details><summary>Question</summary><div class="body"><div>Answer</div></div></details></div> — use for FAQs, policies and service details.
- lk-bento: an asymmetric six-column grid whose first tile is featured large; tiles lift on hover and stack on phones — use for four to seven services, features or gallery items.

A photograph that needs kit classes gets them through its placeholder's "class" field: "lk-hero-img lk-parallax" for a hero photo, "before" or "after" inside lk-compare.
Kit defaults your own CSS can override: .lk-hero is 16:8 with a 16px radius on a near-black ground (for a full-bleed hero, .lk-hero{aspect-ratio:auto;min-height:100svh;border-radius:0}); .lk-compare is 16:9 with a 14px radius; lk-bento tiles and lk-acc dividers are tinted from the text colour; --lk-accent colours the slider's focus ring, and --lk-dur and --lk-ease set the timing.
The kit already turns every pattern off under prefers-reduced-motion, so add no reduced-motion rules of your own for it.`;

// Every new build gets this, after the direction block (which can name
// kit classes). Identical on every build, so it carries its own cache
// breakpoint: one cached copy per direction.
const MOTION_KIT_BLOCK = `Motion kit

Lintel has its own motion library, the motion kit, and it is added to the page for you after you finish. All movement on the site comes from it: use its classes, and never write your own scroll, reveal, parallax, tilt, counter, slider or accordion code, never add @keyframes of your own, and never load an animation library. The one exception is a design direction above that explicitly asks for an animated CSS gradient, which you may write as a single @keyframes.

Mark where the kit goes — once each, on every site, however many patterns you use — and never write the kit's code yourself:
- ${MOTION_KIT_CSS_MARKER} on its own line in the head, immediately before the page's <style> tag. The kit's CSS goes there, ahead of your rules, so your rules can resize and restyle kit elements.
- ${MOTION_KIT_JS_MARKER} on its own line immediately before </body>, after the site's own script. The kit's JS goes there, in its own <script>.
If the brief asks for no motion or animation, leave out both markers and every kit class.

${MOTION_KIT_PATTERNS}`;

// The edit prompts' version. The saved page's kit is folded back into its
// markers before the model sees it (collapseMotionKit), so an edit only
// has to leave them where they are.
const EDIT_MOTION_KIT_RULES = `## Motion kit
The site's movement comes from Lintel's motion kit. In the current website the kit appears only as two markers, ${MOTION_KIT_CSS_MARKER} in the head and ${MOTION_KIT_JS_MARKER} before </body>; its code is put back in after you finish. Keep both markers exactly as and where they are: never remove, move or duplicate them, and never write out the kit's code.
When a change calls for motion, use the kit's classes rather than writing your own animation, scroll, counter or slider code, adding @keyframes, or loading a library. If the page has no markers yet, add them as part of the same change: ${MOTION_KIT_CSS_MARKER} on its own line immediately before the <style> tag, and ${MOTION_KIT_JS_MARKER} on its own line immediately before </body>.

${MOTION_KIT_PATTERNS}`;

function buildSystem(direction) {
  const system = [{ type: 'text', text: SYSTEM_PROMPT, cache_control: { type: 'ephemeral' } }];
  if (direction) system.push({ type: 'text', text: designDirectionBlock(direction) });
  system.push({ type: 'text', text: MOTION_KIT_BLOCK, cache_control: { type: 'ephemeral' } });
  return system;
}

// Runs on the finished page, after the hard-block screen. A build should
// always carry both markers; an edit only once the page uses the kit.
function withMotionKit(result, { projectId, kind }) {
  const { html, css, js, usesKit } = insertMotionKit(result.html);
  if ((!css || !js) && (kind === 'build' || usesKit)) {
    console.warn(`[generate] ${kind} is missing a motion kit marker`, { projectId, css, js, insertedAnyway: usesKit });
  }
  return { ...result, html };
}
...
<!--LUMEN-PLAN {"message":"...","pages":["Home","About"],"palette":"short description","fonts":"the pairing you chose","layout":"the composition you chose","voice":"the tone of the copy","suggestions":["...","..."],"brandColor":"#RRGGBB","headingFont":"Exact Font Name","business":{"name":"...","type":"...","street":"...","locality":"...","region":"...","postcode":"...","country":"...","phone":"...","email":"...","hours":["Mo-Fr 09:00-17:30","Sa 10:00-16:00"]}} -->
...
"brandColor" is the site's dominant brand colour as a hex code, and "headingFont" the exact Google Fonts family name used for headings — both are used to generate the preview card shown when the site is shared, so they must match what you actually used.
"business" carries the real-world facts about the business, and is used to build structured data that search engines read. It has ONE rule, and it matters more than completeness: include a key ONLY if the brief actually states that fact. Never guess, never fill a plausible-looking address, phone number or opening hours, and never carry over an example from these instructions. Omit any key you were not told — an absent field costs nothing, an invented one publishes a false address or a phone number belonging to someone else. Every one of these values is also checked against the visible text of the page you generate before it is used, and silently discarded if it does not appear there, so a fact you did not put on the page is wasted effort either way. If the brief gives no business facts at all, include just "name". "type" must be a schema.org LocalBusiness subtype (Restaurant, CafeOrCoffeeShop, HairSalon, Plumber, Dentist, Florist, ClothingStore, Hotel and so on) or omitted if none fits. "hours" entries must be exactly in the form "Mo-Fr 09:00-17:30" or "Sa 10:00-16:00", 24-hour, or omitted.
...
/**
 * An edit that trips a hard-block rule gets one more go, with the
 * problem named, before we accept it.
 *
 * Without this the model can take a live site offline by itself: the
 * edit saves, screenLiveSite finds the block, and the customer's site
 * goes dark because they asked for a chat widget. A retry costs tokens;
 * an unpublished site costs them business.
 *
 * One retry only. If the model does it twice with the rule spelled out,
 * more attempts are unlikely to help and the customer is waiting.
 */
// A stage line the builder shows while the retry runs. The client
// strips it from the stream and pins it, so the heuristics that
// normally derive a stage from the HTML cannot overwrite it.
export const RETRY_STAGE_TEXT = 'Adjusting that so it can be published…';

function retryStageMarker() {
  return `
<!--LUMEN-STAGE ${JSON.stringify({ text: RETRY_STAGE_TEXT })} -->
`;
}

function hardBlockRetryNote(block) {
  return [
    'IMPORTANT — your previous attempt was rejected automatically and cannot be published.',
    `It contained ${block.label}${block.detail ? ` (${block.detail})` : ''}.`,
    'Produce the requested result again WITHOUT it.',
    'Do not add a script or iframe loading from an outside host, do not write inline JavaScript that sends data to another site, and do not add a password field or point a form at another company.',
    "If the customer asked for a third-party widget, leave a clearly-labelled placeholder instead and use the plan comment's \"message\" to ask them to paste the provider's own embed snippet.",
  ].join(' ');
}

/**
 * Runs an attempt, and if the result trips a hard-block rule, runs it
 * once more with the problem named.
 *
 * Shared by builds and edits because the failure is the same either
 * way: the model writes something that cannot be published, and the
 * customer pays for it — a new build that can't go live, or a live site
 * taken offline. One retry costs tokens; neither of those outcomes is
 * cheaper.
 *
 * Only one retry. If the model does it twice with the rule spelled out,
 * a third attempt is unlikely to help and the customer is waiting.
 *
 * @param {(opts: {blockNote?: string, isRetry: boolean}) => Promise<object>} attempt
 */
async function withHardBlockRetry(attempt, { projectId, kind, onRetry }) {
  const first = await attempt({ isRetry: false });

  const block = findHardBlock(first.html);
  if (!block) return first;

  console.warn(`[generate] ${kind} tripped a hard block — retrying once`, {
    projectId,
    ruleId: block.id,
    detail: block.detail || null,
  });

  // Tell the browser what is happening before the quiet part. The retry
  // does not stream, so without this the preview sits untouched for a
  // whole generation and looks like it has died.
  if (onRetry) {
    try {
      onRetry();
    } catch (err) {
      // A failed notice is cosmetic. Never let it cost us the retry.
      console.error('[generate] could not send the retry notice', err);
    }
  }

  const retry = await attempt({ blockNote: hardBlockRetryNote(block), isRetry: true });
  const usage = addUsage(first.usage, retry.usage);
  const stillBlocked = findHardBlock(retry.html);

  if (stillBlocked) {
    // Give up and hand it back. Downstream behaviour is unchanged from
    // before this guard existed: an edit saves and the offline path
    // takes the site down, a new build saves and is refused at publish.
    console.error(`[generate] ${kind} retry still blocked — falling through to the normal path`, {
      projectId,
      firstRuleId: block.id,
      retryRuleId: stillBlocked.id,
      detail: stillBlocked.detail || null,
    });
    return { ...retry, usage, hardBlockRetried: block.id, hardBlockUnresolved: stillBlocked.id };
  }

  console.warn(`[generate] ${kind} retry cleared the block`, { projectId, ruleId: block.id });
  return { ...retry, usage, hardBlockRetried: block.id };
}

async function runEdit(args) {
  // The model edits the page with the kit folded back to its markers:
  // patches and the similarity guard both work on that version, and the
  // current kit goes back in once the result has been screened.
  const previousHtml = collapseMotionKit(args.previousHtml);
  const result = await withHardBlockRetry(
    ({ blockNote }) => editOnce({ ...args, previousHtml, blockNote }),
    { projectId: args.projectId, kind: 'edit', onRetry: args.onRetryNotice }
  );
  return withMotionKit(result, { projectId: args.projectId, kind: 'edit' });
}

// The prompts ask for the current year in copyright notices, but the model
// has no clock and guesses. Worked out per request, not at module load, so
// a long-lived instance can't carry last year across New Year.
function currentYear() {
  return new Date().getUTCFullYear();
}

async function editOnce({ brief, previousHtml, images, modelKey, projectId, userId, plan, signal, blockNote }) {
 ...
 * style        - a DESIGN_DIRECTION_NAMES entry the customer picked, or null to choose one
 */
export async function generateSite({ brief, modelKey = 'opus', previousHtml = null, images = [], projectId, userId, plan, style = null }) {
  ...
  const direction = chooseDesignDirection({ brief, style, plan, projectId });
  const result = await withHardBlockRetry(
    ({ blockNote }) => buildOnce({ brief, images, modelKey, projectId, userId, plan, direction, blockNote }),
    { projectId, kind: 'build' }
  );
  return withMotionKit(result, { projectId, kind: 'build' });
}

async function buildOnce({ brief, images, modelKey, projectId, userId, plan, direction, blockNote }) {
  const text = `Build a website. The current year is ${currentYear()}. Brief: ${brief}`
    + (blockNote ? `\n\n---\n\n${blockNote}` : '');
  const userContent = images.length
    ? [
        ...images.map((img) => ({ type: 'image', source: { type: 'base64', media_type: img.mediaType, data: img.data } })),
    ...
    : text;
  const system = buildSystem(direction);
 ...
 */
export async function streamSite({ brief, modelKey = 'opus', previousHtml = null, images = [], onChunk, signal, projectId, userId, plan, style = null }) {
  const editing = !!previousHtml;

  // Edits never stream their content, but they can still send this one
  // line -- the customer is watching the same screen either way.
  const onRetryNotice = onChunk ? () => onChunk(retryStageMarker(), '') : null;

  if (editing) {
    return await runEdit({
      brief, previousHtml, images, modelKey, projectId, userId, plan, signal, onRetryNotice,
    });
  }

  // The retry deliberately does NOT stream. The browser has already
  // rendered the first attempt as a live preview, and streaming a
  // second document into the same buffer would show the customer two
  // sites being written over each other. The client takes its final
  // content from the meta line rather than the buffer, so the retry's
  // result still lands correctly -- the preview just holds still while
  // it runs.
  const direction = chooseDesignDirection({ brief, style, plan, projectId });
  const result = await withHardBlockRetry(
    ({ blockNote, isRetry }) => streamBuildOnce({
      brief, images, modelKey, projectId, userId, plan, signal, direction, blockNote,
      onChunk: isRetry ? null : onChunk,
    }),
    { projectId, kind: 'build', onRetry: onRetryNotice }
  );
  // The live preview streamed the markers; the kit arrives with the final
  // code in the meta line, which the client takes over the buffer.
  return withMotionKit(result, { projectId, kind: 'build' });
}

async function streamBuildOnce({ brief, images, modelKey, onChunk, signal, projectId, userId, plan, direction, blockNote }) {
  const text = `Build a website. The current year is ${currentYear()}. Brief: ${brief}`
    + (blockNote ? `\n\n---\n\n${blockNote}` : '');
...
function extractPlan(html) {
  ...
  try {
    ...
    const clean = {};
    ['message', 'pages', 'palette', 'fonts', 'layout', 'voice', 'changed', 'suggestions',
     'brandColor', 'headingFont'].forEach((k) => {
    ...
    // business feeds structured data, so it is whitelisted key by key
    // and length-capped: whatever the model puts here ends up in front
    // of a search engine as a factual claim about a real place.
    const b = raw.business;
    if (b && typeof b === 'object' && !Array.isArray(b)) {
      const biz = {};
      const caps = {
        name: 120, type: 40, street: 120, locality: 80,
        region: 80, postcode: 20, country: 40, phone: 40, email: 120,
      };
      Object.entries(caps).forEach(([k, cap]) => {
        if (b[k] == null) return;
        const v = String(b[k]).trim().slice(0, cap);
        if (v) biz[k] = v;
      });
      if (Array.isArray(b.hours)) {
        const hours = b.hours.map((h) => String(h).trim().slice(0, 40)).filter(Boolean).slice(0, 7);
        if (hours.length) biz.hours = hours;
      }
      if (Object.keys(biz).length) clean.business = biz;
    }
