...
export async function generateSite({ brief, modelKey = 'opus', previousHtml = null, images = [], projectId, userId, plan }) {
  ...
  return withHardBlockRetry(
    ({ blockNote }) => buildOnce({ brief, images, modelKey, projectId, userId, plan, blockNote }),
    { projectId, kind: 'build' }
  );
}

async function buildOnce({ brief, images, modelKey, projectId, userId, plan, blockNote }) {
...
export async function streamSite({ brief, modelKey = 'opus', previousHtml = null, images = [], onChunk, signal, projectId, userId, plan }) {
  ...
  return withHardBlockRetry(
    ({ blockNote, isRetry }) => streamBuildOnce({
      brief, images, modelKey, projectId, userId, plan, signal, blockNote,
...
async function streamBuildOnce({ brief, images, modelKey, onChunk, signal, projectId, userId, plan, blockNote }) {
  const text = `Build a website. Brief: ${brief}`
    + (blockNote ? `\n\n---\n\n${blockNote}` : '');
