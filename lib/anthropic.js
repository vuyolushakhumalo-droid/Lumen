...
- Never invent external URLs — no fabricated booking, ordering, or social links to real third-party domains (OpenTable, Deliveroo, Instagram, etc.) unless the request supplies the real link. Booking or ordering buttons should scroll to an on-page section or use a no-op form with an inline confirmation.
...
function isNearTotalReplacement(previousHtml, newHtml) {
  ...
  const titleChanged = !!(prevTitle && newTitle) && prevTitle !== newTitle;
...
async function runEdit({ brief, previousHtml, images, modelKey, projectId, userId, plan, signal }) {
  ...
  for (let attempt = 0; attempt < 2 && !patched; attempt++) {
    ...
    if (patches) patched = applyPatches(previousHtml, patches);
  }
  ...
  if (!cleaned) throw new Error('The model did not return a usable page');
  ...
  return {
    ...
    imagesQuotaExhausted, usedFallback, needsConfirmation,
  };
}
 ...
 */
export async function generateSite({ brief, modelKey = 'sonnet', previousHtml = null, images = [], projectId, userId, plan }) {
 ...
 */
export async function streamSite({ brief, modelKey = 'sonnet', previousHtml = null, images = [], onChunk, signal, projectId, userId, plan }) {
  const editing = !!previousHtml;
