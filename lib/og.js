...
// Fallback is Lintel's own cyan on near-black -- deliberately ours, not
// a guess at theirs. A wrong brand colour looks like a mistake; a
// neutral one just looks plain.
const FALLBACK_BG = '#0C1119';
const FALLBACK_FG = '#F5F6F9';
...
export async function renderOgImage({ name, tagline, brandColor, headingFont }) {
  ...
  const ink = normaliseHex(brandColor) ? readableInk(bg) : FALLBACK_FG;
  const muted = ink === '#FFFFFF' ? 'rgba(255,255,255,0.72)' : 'rgba(16,20,24,0.68)';

  const title = String(name || 'Website').trim().slice(0, 70);
  const sub = String(tagline || '').trim().slice(0, 110);
