...
/**
 * Deletes a version row that was inserted ahead of a current_code write
 * which then didn't land (a lock conflict or a hard failure). Best-effort:
 * an orphan version is only ever visible via /restore and can't corrupt
 * anything, so a failure here is logged and swallowed rather than raised
 * -- it must never mask the error or conflict response it was called from.
 */
export async function rollbackVersion(admin, versionId) {
  if (!versionId) return;
  try {
    await admin.from('versions').delete().eq('id', versionId);
  } catch (err) {
    console.error('[usage] failed to roll back orphaned version', versionId, err);
  }
}
...
export async function getUsageSnapshot(admin, profile) {
  ...
  // Best-effort -- a failure here shouldn't break the build-allowance
  // snapshot this also carries. Mirrors lib/images.js's own count
  // exactly, so what's shown here always matches what's enforced.
  let imagesUsed = 0;
  try {
    ...
    console.error('[usage] failed to count images this month', err);
  }
  const imagesLimit = plan?.monthlyImages || 0;
  ...
  return {
    ...
    imagesUsed,
    imagesLimit,
    imagesLeft: Math.max(0, imagesLimit - imagesUsed),
