...
async function logImageGeneration(admin, { userId, projectId, status }) {
  try {
    await admin.from('image_generations').insert({
      user_id: userId, project_id: projectId,
      model: IMAGE_MODEL, quality: IMAGE_QUALITY, status,
    });
  } catch (err) {
...
async function attemptOne(m, { projectId, userId, admin }) {
  ...
  try {
    ...
    await logImageGeneration(admin, { userId, projectId, status: 'success' });
    ...
    await logImageGeneration(admin, { userId, projectId, status: 'failed' });
