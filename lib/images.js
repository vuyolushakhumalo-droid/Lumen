...
// The placeholder's optional "class" — how a photo gets motion kit classes
// such as lk-hero-img or lk-parallax. Plain class names only.
function classAttr(raw) {
  const names = String(raw || '').split(/\s+/).filter((c) => /^[A-Za-z0-9_-]{1,40}$/.test(c)).slice(0, 6);
  return names.length ? ` class="${names.join(' ')}"` : '';
}

function fallbackBlock(alt, aspect, cls = '') {
  const ratio = RATIO_BY_ASPECT[aspect] || RATIO_BY_ASPECT.square;
  return `<div role="img"${cls} aria-label="${escapeAttr(alt)}" style="aspect-ratio:${ratio};border-radius:14px;background:linear-gradient(135deg,#d8dde3,#eef0f3)"></div>`;
}

function fallbackFromMatch(m) {
  let alt = '', aspect = 'square', cls = '';
  try { const spec = JSON.parse(m[1]); alt = spec.alt; aspect = normalizeAspect(spec.aspect); cls = classAttr(spec.class); } catch (e) {}
  return fallbackBlock(alt, aspect, cls);
}
...
async function attemptOne(m, { projectId, userId, admin }) {
  ...
  try {
    ...
    return { raw: m[0], replacement: `<img src="${url}"${cls} alt="${escapeAttr(alt)}" loading="lazy">` };
  } catch (err) {
    ...
    return { raw: m[0], replacement: fallbackBlock(alt, aspect, cls) };
  }
}
