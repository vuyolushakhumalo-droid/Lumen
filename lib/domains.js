...
export async function addDomainToVercel(domain) {
  ...
  try {
    ...
    if (!ok && body?.error?.code !== 'domain_already_exists') {
      logError('[domains] vercel add failed', body);
      return { automated: false, error: body?.error?.message };
    }
    return { automated: true };
  } catch (err) {
    logError('[domains] vercel add call failed', err);
...
export async function removeDomainFromVercel(domain) {
  ...
  try {
    ...
    if (!ok && status !== 404) {
      logError('[domains] vercel remove failed', body);
      return { automated: false, error: body?.error?.message };
    }
    return { automated: true };
  } catch (err) {
    logError('[domains] vercel remove call failed', err);
    return { automated: false };
  }
}
...
export async function checkDomainWithVercel(domain) {
  ...
  try {
    ...
    logError('[domains] vercel check failed', domain, err);
    return {
      status: 'error',
  ...
  if (reg.status === 404) {
    ...
    if (added.automated) {
      try {
        ...
        logError('[domains] vercel re-lookup failed', domain, err);
      }
    }
  }
  ...
  if (!reg.ok) {
    logError('[domains] vercel domain lookup failed', reg.body);
    return {
      status: 'error',
...
export async function refreshDomainStatus(admin, site) {
  ...
  try {
    const { error } = await admin
      ...
      .eq('id', site.id);
    if (error) logError('[domains] could not store domain status', site.id, error);
  } catch (err) {
    logError('[domains] could not store domain status', site.id, err);
  }
