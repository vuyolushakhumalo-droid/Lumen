...
/**
 * Cuts to a word boundary rather than mid-word.
 *
 * A hard slice produced "…hardy perennials, houseplan" on a real card:
 * a truncated word reads as a rendering fault, where an ellipsis reads
 * as a summary. Falls back to a hard cut only when there is no space
 * late enough to break on, so one very long word can't eat the line.
 */
...
export async function renderOgImage({ name, tagline, brandColor, headingFont }) {
  ...
  const title = trimTo(decodeEntities(name), 70) || 'Website';
  const sub = trimTo(decodeEntities(tagline), 110);
